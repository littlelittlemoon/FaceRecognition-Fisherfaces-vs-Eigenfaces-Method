...
def myPCA(A):
    ...
    # compute mean, and subtract mean from every column （均值化）
    [r, c] = A.shape
    m = np.mean(A, 1)
    A = A - np.transpose(np.tile(m, (c, 1)))

    # Scatter matrix (计算散布矩阵)
    B = np.dot(np.transpose(A), A)   # B = A'A
    [d, v] = linalg.eig(B)  # 计算散布矩阵B的特征值特征向量
    # v is in descending sorted order

    # compute eigenvectors(W) of scatter matrix
    W = np.dot(A,v)
    Wnorm = ComputeNorm(W)
    ...
    # omit last column, which is the nullspace
    W = W2[:,0:-1]      
